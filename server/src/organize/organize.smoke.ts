 ...
 *   npx tsx src/organize/organize.smoke.ts
...
async function playOut(id: string, organizerId: string) {
  ...
  return org.getTournament(id)!;
}

// ---- A) paid 4-team cup: pot conservation ----
console.log('\n— paid 4-team cup —');
const players = ['ana', 'ben', 'cid', 'dee'].map((h) => createAccount(h));
const org1 = players[0].account.id;
let t = org.createTournament({ organizerId: org1, name: 'Smoke Cup', maxPlayers: 4, entryFee: 100, splitBps: [7000, 3000] });
...
const afterJoin = players.reduce((a, p) => a + (getAccount(p.account.id)!.points), 0);
check('each debited 100 (sum = 3600)', afterJoin === 3600, afterJoin);

t = org.startTournament({ tournamentId: t.id, organizerId: org1 });
...
const totalPoints = players.reduce((a, p) => a + getAccount(p.account.id)!.points, 0);
...
const host = createAccount('host').account.id;
let b = org.createTournament({ organizerId: host, name: 'Bye Cup', maxPlayers: 4, entryFee: 0 });
b = org.addEntrant({ tournamentId: b.id, organizerId: host, name: 'Brazil' });
b = org.addEntrant({ tournamentId: b.id, organizerId: host, name: 'France' });
b = org.addEntrant({ tournamentId: b.id, organizerId: host, name: 'Japan' });
check('3 entrants', b.participantCount === 3, b.participantCount);
b = org.startTournament({ tournamentId: b.id, organizerId: host, seeding: 'join' });
